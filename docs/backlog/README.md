...
|-------|-------------|----------|------------|----|-------|
| 0     | Outbox tables; tokenization smoke; no-magic-numbers gate | KMS HSM keyrings; VPC-SC perimeters | Privacy/Security Officers; PHI inventory; P&P content; background-check policy; COPPA decision | Design system commit; user-research program | GCP projects, IaC, CI/CD, observability backbone |
| 1     | Format adapter, DQ Required tier, state machine + grace period, Verification API skeleton + availability SLO, replay-safe ingestion | Latency floor; JWT verifier; redaction scanner LIVE | NPP authoring; first BAA; partner data sharing agreement; workforce departure procedures | Verification failure UX; lockout recovery service blueprint | Datastream→BigQuery; cross-region backup replication; audit chain validator |
| 2     | Splink integration + model lifecycle; reviewer queue; deletion workflow; late-arriving + replay orchestrator; sync-detok contract | Brute force progression; hash chain + external anchor; two-person rule | Federal breach notification machinery (BR-1002..1008); 50-state breach matrix; member rights workflows; training program LIVE | Reviewer interface; audit-forensic-search UX; member portal phase-1 | GCS Bucket Lock; cross-org Compliance project; operational history window |
